...
function [lower_bounds_sddp,upper_bounds_sddp,time_sddp]=inexact_stodcup_quadratic_cut_selection(T,n,M,iter_stodcup,nb_iter_max,xis,Us,Psis,tol,x0,Minit,probabilities,accuracies,iter_selection_init,iter_selection_length)
...
while End_Algo
    ...
    if (iter<=iter_stodcup)
        ...
        elseif (iter<=(iter_selection_init+iter_selection_length-1))
            prob.buc=[inf*ones(counters(1,1),1);hs{1,1}{1,1};inf*ones(iter_selection_init,1)];
            prob.blc=[cs{1,1}{1,1};-inf*ones(counters(1,1),1);thetas{1,1}];
            prob.a=sparse([subia{1,1}+2*counters(1,1),subistodcup{1,1}{1,1},subistodcupg{1,1}{1,1}+counters(1,1)],[subja{1,1},subjstodcup{1,1}{1,1},subjstodcupg{1,1}{1,1}],[valija{1,1},valijstodcup{1,1}{1,1},valijstodcupg{1,1}{1,1}],2*counters(1,1)+iter_selection_init,n+2);
        else
            prob.buc=[inf*ones(counters(1,1),1);hs{1,1}{1,1};inf*ones(iter-iter_selection_length+1,1)];
            prob.blc=[cs{1,1}{1,1};-inf*ones(counters(1,1),1);thetas{1,1}];
            prob.a=sparse([subia{1,1}+2*counters(1,1),subistodcup{1,1}{1,1},subistodcupg{1,1}{1,1}+counters(1,1)],[subja{1,1},subjstodcup{1,1}{1,1},subjstodcupg{1,1}{1,1}],[valija{1,1},valijstodcup{1,1}{1,1},valijstodcupg{1,1}{1,1}],2*counters(1,1)+iter-iter_selection_length+1,n+2);
        end
        ...
        for t=2:T-1
            ...
            elseif (iter<=(iter_selection_init+iter_selection_length-1))                
                prob.buc=[inf*ones(counters(t,Index),1);hs{1,t}{1,Index};inf*ones(iter_selection_init,1)];
                prob.blc=[bs{1,t}{1,Index}*trial_states{1,t-1}+cs{1,t}{1,Index};-inf*ones(counters(t,Index),1);thetas{1,t}];
                prob.a=sparse([subia{1,t}+2*counters(t,Index),subistodcup{1,t}{1,Index},subistodcupg{1,t}{1,Index}+counters(t,Index)],[subja{1,t},subjstodcup{1,t}{1,Index},subjstodcupg{1,t}{1,Index}],[valija{1,t},valijstodcup{1,t}{1,Index},valijstodcupg{1,t}{1,Index}],2*counters(t,Index)+iter_selection_init,n+2);
            else
                prob.buc=[inf*ones(counters(t,Index),1);hs{1,t}{1,Index};inf*ones(iter-iter_selection_length+1,1)];
                prob.blc=[bs{1,t}{1,Index}*trial_states{1,t-1}+cs{1,t}{1,Index};-inf*ones(counters(t,Index),1);thetas{1,t}];
                prob.a=sparse([subia{1,t}+2*counters(t,Index),subistodcup{1,t}{1,Index},subistodcupg{1,t}{1,Index}+counters(t,Index)],[subja{1,t},subjstodcup{1,t}{1,Index},subjstodcupg{1,t}{1,Index}],[valija{1,t},valijstodcup{1,t}{1,Index},valijstodcupg{1,t}{1,Index}],2*counters(t,Index)+iter-iter_selection_length+1,n+2);
            end
    ...
    for t=T:-1:2
        if (t==T)
            ...
            for j=1:M
                ...
                elseif ((iter+1)<=(iter_selection_init+iter_selection_length-1))
                    prob.buc=[inf*ones(counters(t,j),1);hs{1,t}{1,j};inf*ones(iter_selection_init,1)];
                    prob.blc=[bs{1,t}{1,j}*trial_states{1,t-1}+cs{1,t}{1,j};-inf*ones(counters(t,j),1);thetas{1,t}];
                    prob.a=sparse([subia{1,t}+2*counters(t,j),subistodcup{1,t}{1,j},subistodcupg{1,t}{1,j}+counters(t,j)],[subja{1,t},subjstodcup{1,t}{1,j},subjstodcupg{1,t}{1,j}],[valija{1,t},valijstodcup{1,t}{1,j},valijstodcupg{1,t}{1,j}],2*counters(t,j)+iter_selection_init,n+2);
                else
                    prob.buc=[inf*ones(counters(t,j),1);hs{1,t}{1,j};inf*ones(iter-iter_selection_length+1,1)];
                    prob.blc=[bs{1,t}{1,j}*trial_states{1,t-1}+cs{1,t}{1,j};-inf*ones(counters(t,j),1);thetas{1,t}];
                    prob.a=sparse([subia{1,t}+2*counters(t,j),subistodcup{1,t}{1,j},subistodcupg{1,t}{1,j}+counters(t,j)],[subja{1,t},subjstodcup{1,t}{1,j},subjstodcupg{1,t}{1,j}],[valija{1,t},valijstodcup{1,t}{1,j},valijstodcupg{1,t}{1,j}],2*counters(t,j)+iter-iter_selection_length+1,n+2);              
                end                
                param.MSK_DPAR_INTPNT_TOL_REL_GAP=accuracies(iter);
                [~,res]=mosekopt('minimize echo(0)',prob,param);
                sol=res.sol.itr.xx;
        ...
        elseif ((iter+1)<=(iter_selection_init+iter_selection_length-1))
           valija{1,t-1}=[valija{1,t-1}(n+2:(n+1)*iter_selection_init),1,-slope'];
           thetas{1,t-1}=[thetas{1,t-1}(2:iter_selection_init-1);intercept];
        else
           subia{1,t-1}=[subia{1,t-1},(iter-iter_selection_length+1)*ones(1,n+1)];
           subja{1,t-1}=[subja{1,t-1},[2:1:n+2]];
           valija{1,t-1}=[valija{1,t-1},1,-slope'];
           thetas{1,t-1}=[thetas{1,t-1};intercept];
        end
        ...
        while ((t>=2)&&(contloop))
            ...
            while((j<=M)&&(contloop)) 
               ...
               elseif (strcmp(solsta,'MSK_PRO_STA_UNKNOWN')==1)
                  ...
                  slope=slope+dual;
                  intercept=intercept+sol(1)+sol(2)-abs(sol.pobjval-sol.dobjval)-dual'*trial_states{1,t-1};
               end
               taux1=taux1+toc;
               j=j+1;
            end
